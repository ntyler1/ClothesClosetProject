...
public class RemoveClothingItemView extends View {
	...
	private VBox createFormContent() {
		...
		barcode.setOnKeyReleased(event -> {
			if (barcode.getText().length() == 0)
				...
				try {
					...
					case 0:
						gender.setStyle(
								"-fx-border-color: firebrick; -fx-background-color: white; -fx-selection-bar:lightgreen;");
						break;
					case 1:
		...
		gender.setItems(FXCollections.observableArrayList(new Gender("Male", 0), new Gender("Female", 1)));
		...
		articleType.setOnAction((event) -> {
			...
			if (barcode.getText().length() < 8)
				barcode.setText("00000001");
			barcode.setText(barcode.getText().substring(0, 1)
					+ articleType.getSelectionModel().getSelectedItem().getBarcodePrefix()
					+ barcode.getText().substring(3));
		});
		...
		color1.setOnAction((event) -> {
			...
			if (barcode.getText().length() < 8)
				barcode.setText("00000001");
			barcode.setText(barcode.getText().substring(0, 3)
					+ color1.getSelectionModel().getSelectedItem().getBarcodePrefix() + barcode.getText().substring(5));
		});
		...
		size = new ComboBox<>();
		size.setItems(FXCollections.observableArrayList("S", "M", "L", "XL", "XXL"));
												...
												if (donorLNameStr.length() > 0) {
													...
													if (donorPhoneStr.length() > 0) {
														...
														if (donorEmailStr.length() > 0) {
															...
															props.setProperty("Gender", gender.getSelectionModel()
																	.getSelectedItem().getName());
															props.setProperty("Size",
																	size.getSelectionModel().getSelectedItem());
															props.setProperty("ArticleType",
																	articleType.getSelectionModel().getSelectedItem()
																			.getBarcodePrefix());
															props.setProperty("Color1", color1.getSelectionModel()
																	.getSelectedItem().getBarcodePrefix());
															props.setProperty("Color2", color2.getSelectionModel()
																	.getSelectedItem().getBarcodePrefix());
														...
														displayErrorMessage(
																"ERROR: Please Enter a Donor Phone Number!");
													}
												} else {
	...
	public void populateFields() throws InvalidPrimaryKeyException, MultiplePrimaryKeysException
	{
		...
		String note = (String)myModel.getState("Notes");
		if (note != null)
		{
			notes.setText(note);
		...
		String fname = (String)myModel.getState("FirstName");
		if (fname != null)
		{
			donorFName.setText(fname);
              donorFName.setDisable(true);
		}
		
		String lname = (String)myModel.getState("LastName");
		if (lname != null)
		{
			donorLName.setText(lname);
		...
		String mail = (String)myModel.getState("Email");
		if (mail != null)
		{
			donorEmail.setText(mail);
		...
		String sze = (String)myModel.getState("Size");
		if (sze != null)
		{
			size.setValue(sze);
